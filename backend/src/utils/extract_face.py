...
def main():
    ...
    try:
        try:
         ...
         print(json.dumps({
             "status": "error", 
             "message": f"tflite_runtime or tensorflow not installed. Details: {str(e)}. Python: {sys.executable}"
         }))
         sys.exit(1)
    ...
    # 2. Detect face using OpenCV Haar Cascade (removes mediapipe dependency which breaks across versions)
